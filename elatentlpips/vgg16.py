...
# add
from .elatentlpips_helper.models import VGG16_Latent, VGG16_Latent_BN, VGG16_Latent_GN
import torch.nn as nn

class CalibratedLatentVGG16BN(torch.nn.Module):
    def __init__(self, num_latent_channels, encoder, pretrained=False, requires_grad=True, vgg_pth=None):
        super(CalibratedLatentVGG16BN, self).__init__()
        model = LatentVGG16BN(num_latent_channels, requires_grad=True)

        if encoder == 'taesd' and vgg_pth == None:
            raise ValueError('VGG_pth 값을 넣으세요.')

        if pretrained and encoder != 'taesd':
            # Load checkpoint
        ...
        elif pretrained and encoder == 'taesd' and vgg_pth != None:
            model = TAESDLatentVGG16BN(num_latent_channels=16)
            ckpt = load_file(vgg_pth, device='cpu')
            ...
            for k, v in ckpt.items():
                ...
                if "linear" in name:
                    num = int(name.split('.')[0][-1]) - 1
                    suffix = ".".join(name.split('.')[1:])
                    name = f"lin{num}.{suffix}"

                new_state_dict[name] = v
                # if "lin" in name and "model" not in name:
                #     name = name.replace(".weight", ".model.1.weight")
                #     name = name.replace(".bias", ".model.1.bias")
            # model.load_state_dict(new_state_dict)
        ...
        self.slice1 = nn.Sequential(*vgg_pretrained_features[0:6])
        self.slice2 = nn.Sequential(*vgg_pretrained_features[6:12])
        self.slice3 = nn.Sequential(*vgg_pretrained_features[12:21])
        self.slice4 = nn.Sequential(*vgg_pretrained_features[21:31])
        self.slice5 = nn.Sequential(*vgg_pretrained_features[31:41])
...
class LatentVGG16BN(torch.nn.Module):
    def __init__(self, num_latent_channels, requires_grad=True, pretrained=False):
        ...
        delete_idx = [i for i, layer in enumerate(self.model.features) if isinstance(layer, torch.nn.MaxPool2d)]
...
class TAESDLatentVGG16BN(torch.nn.Module):
    def __init__(self, num_latent_channels, requires_grad=True, pretrained=False):
        super(TAESDLatentVGG16BN, self).__init__()
        self.model = VGG16_Latent_BN(in_channels=num_latent_channels)
