...
def _init():
    ...
    if _plugin is None:
        ...
        except: # ⬅️ except 블록 추가 (컴파일 실패 처리)
            # 실패 시 False를 반환하여 순수 Python 구현으로 우회합니다.
            # print(f"Warning: Failed to compile 'upfirdn2d_plugin'. Falling back to CPU/Ref implementation.", flush=True)
            return False 
