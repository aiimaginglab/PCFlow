...
class ELatentLPIPS(nn.Module):
    def __init__(self, pretrained=True, net='vgg16', elatentlpips=True, spatial=False, 
        ...
        verbose=True, encoder="sd3", augment="bg", taesd_vgg_pth=None, taesd_lpips_pth=None):
        ...
        self.taesd_vgg_pth = taesd_vgg_pth
        self.taesd_lpips_pth = taesd_lpips_pth
        ...
        if self.encoder == "sd15":
            ...
            num_latent_channels = 16
        elif self.encoder == "taesd":
            num_latent_channels = 16
            if taesd_vgg_pth == None and taesd_lpips_pth == None:
                raise ValueError("taesd_vgg_pth 와 taesd_lpips_pth 인자가 None 입니다.")
    
        # self.net = net_type(num_latent_channels, self.encoder, pretrained=not self.pnet_rand, requires_grad=self.pnet_tune, vgg_pth=self.taesd_vgg_pth)
        ...
        if(elatentlpips):
            if(pretrained):
                if self.encoder in ["sd15", "sd21", "sdxl", "sd3", "flux"]:
                    ...
                    if not os.path.isfile(taesd_lpips_pth):
                        raise ValueError(f'{taesd_lpips_pth} is not existed.')

                    backbone = VGG16_Latent_BN(in_channels=16)
                    backbone.load_state_dict(load_file(self.taesd_vgg_pth))
                    ...
                    ckpt = load_file(self.taesd_lpips_pth, device='cpu')
