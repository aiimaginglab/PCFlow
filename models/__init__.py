...
from .lunet import LUnetAdapter
from .wrapper import DecoderFeatureExtractor, TAESDWrapper, TAESDDecoderFeatureExtractor
...
def create_network(config):
    ...
    if "DiT" not in config.model_type:
        if config.model_type == "lunet":
            return LUnetAdapter(
                in_channels=config.num_in_channels,
                out_channels=config.num_out_channels,
                hid_channels=config.nf,
                ch_mult=config.ch_mult,
                n_mid_blocks=config.n_mid_blocks
            )
